...
export default function HistoryPage() {
  ...
  return (
    <div className="container">
      <div className="page-header"><h1>📜 Verlauf</h1></div>

      <div className="tabs">
        <button className={`tab ${tab === 'posts' ? 'active' : ''}`} onClick={() => setTab('posts')}>Beiträge ({posts.length})</button>
        <button className={`tab ${tab === 'items' ? 'active' : ''}`} onClick={() => setTab('items')}>Anzeigen ({items.length})</button>
        <button className={`tab ${tab === 'jobs' ? 'active' : ''}`} onClick={() => setTab('jobs')}>Jobs ({jobs.length})</button>
        <button className={`tab ${tab === 'courses' ? 'active' : ''}`} onClick={() => setTab('courses')}>Kurse ({courses.length})</button>
      </div>

      {loading ? <p style={{ textAlign: 'center', color: 'var(--text-muted)' }}>Laden...</p> : (
        <>
          {tab === 'posts' && (posts.length === 0 ? <div className="empty-state"><p>Noch keine Beiträge.</p></div> : posts.map(p => (
          ...
          {tab === 'items' && (items.length === 0 ? <div className="empty-state"><p>Noch keine Anzeigen.</p></div> : items.map(i => (
          ...
          {tab === 'jobs' && (jobs.length === 0 ? <div className="empty-state"><p>Noch keine Jobs.</p></div> : jobs.map(j => (
          ...
          {tab === 'courses' && (courses.length === 0 ? <div className="empty-state"><p>Noch keine Kurse.</p></div> : courses.map(c => (
