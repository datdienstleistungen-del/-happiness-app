...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!usage) {
      console.log("[NEXUS] Inserting usage");
      try {
      ...
      if (newCount > MAX_REQUESTS) return { statusCode: 429, body: JSON.stringify({ error: "Rate limit exceeded." }) };
      
      console.log("[NEXUS] Updating usage");
      try {
